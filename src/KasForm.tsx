...
const StatusMessage = ({ type, message }: { type: FormStatusType['type'], message: string | null }) => {
    ...
    const config = {
        success: { bgColor: "bg-green-50 border-green-400", textColor: "text-green-800", icon: <i className="fa-solid fa-circle-check"></i> },
        error: { bgColor: "bg-red-50 border-red-400", textColor: "text-red-800", icon: <i className="fa-solid fa-circle-xmark"></i> },
    };
...
export default function KasForm() {
    ...
    const getTanggalLabel = () => {
        if(paymentMethod === 'arrears') return "Bayar Tunggakan Hingga Tanggal Ini";
                    ...
                    {isNewStudent ? (
                        <>
                            <FormField label="Kelas Baru" id="kelas">
                                <IconInput id="kelas" type="text" value={formData.kelas} onChange={handleChange} required disabled={isLoading} placeholder="Contoh: X SIJA 1" icon={<i className="fa-solid fa-users"></i>} />
                            </FormField>
                            
                            <FormField label="Nama Siswa Baru" id="nama">
                                <IconInput id="nama" type="text" value={formData.nama} onChange={handleChange} required disabled={isLoading} placeholder="Contoh: Budi Hartono" icon={<i className="fa-solid fa-user"></i>} />
                            </FormField>
                        </>
                    ) : 
                        (<>
                            <FormField label="Kelas" id="kelas">
                                <Select id="kelas" value={formData.kelas} onChange={handleChange} required disabled={isSiswaLoading || isLoading} icon={<i className="fa-solid fa-users"></i>}>
                                    <option value="" disabled>{isSiswaLoading ? "Memuat kelas..." : "-- Pilih Kelas --"}</option>{Object.keys(siswaData).sort().map((k) => <option key={k} value={k}>{k}</option>)}
                                </Select>
                            </FormField>
                            
                            <FormField label="Cari Nama Siswa" id="nama">
                                <div className="relative" ref={searchRef}>
                                    <IconInput id="nama" type="text" value={formData.nama} onChange={handleChange} onFocus={() => setIsNameDropdownOpen(true)} required disabled={!formData.kelas || isLoading} placeholder="Ketik untuk mencari nama..." icon={<i className="fa-solid fa-user"></i>} autoComplete="off" />{isNameDropdownOpen && filteredNamaOptions.length > 0 && (
                            ...
                            <FormField label="Jumlah Kas"> 
                                <div className="space-y-3">
                                    <div className="flex items-center space-x-6">
                                        ...
                                        {amountOption === 'custom' && (
                                            <IconInput id="jumlah" type="text" inputMode="numeric" value={formatRp(customAmount)} onChange={(e) => setCustomAmount(e.target.value.replace(/[^0-9]/g, ''))} required disabled={isLoading} placeholder="Contoh: Rp 5.000" icon={<i className="fa-solid fa-money-bill-wave"></i>} />)}
                                </div>
                            </FormField>)}
                    </>
                )}
